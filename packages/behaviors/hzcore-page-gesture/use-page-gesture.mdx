...
```shell
yarn add @hzcore/page-gesture
```

## Usage

```
import {usePageGesture} from '@hzcore/page-gesture';
```
...
<Playground>
  {() => {
    ...
    /**
     * A hook to calculate a y translation for the list of pages.
     * @param {number} currentIndex - The current page index.
     * @param {boolean} gesturing - Whether or not some gesture is active.
     * @return {[transform, handler]} - A tuple of react-spring-animated
     *         transform and a callback for updating the transform
     *         based on gesture state.
     */
    const useTransform = (currentIndex, gesturing) => {
      const [transform, setTransform] = useSpring(() => ({
        config: springConfig.stiff,
        transform: `translateY(${currentIndex * -PAGE_SIZE}px)`,
      }));
      React.useEffect(() => {
        if (!gesturing) {
          setTransform({
      ...
      }, [currentIndex, gesturing]);
      const gestureHandler = React.useCallback(
        gesture => {
          if (gesture.gesturing) {
            setTransform({
              immediate: true,
              transform: `translateY(${(gesture.yDelta || 0) +
                currentIndex * -PAGE_SIZE}px)`,
            });
          }
        },
        [currentIndex],
      );
      return [transform, gestureHandler];
    };
    /**
     * A hook for managing pagination.
     * @param {boolean} gesturing - Whether or not a gesture is active.
     * @return {[currentIndex, handler]} - A tuple of the current page index
     *         and a callback for updating the index based on gesture state.
     */
    const usePagination = gesturing => {
      const [currentIndex, setIndex] = React.useState(0);
      const {paginationConfig, paginationHandlers} = React.useMemo(
        () => ({
          paginationConfig: {orientation: VERTICAL},
          paginationHandlers: {
            onNext: () => setIndex(i => Math.min(i + 1, TOTAL_PAGES - 1)),
            onPrevious: () => setIndex(i => Math.max(i - 1, 0)),
            onFirst: () => setIndex(0),
            onLast: () => setIndex(TOTAL_PAGES - 1),
          },
        }),
        [],
      );
      const gestureHandler = usePageGesture(
        paginationHandlers,
        paginationConfig,
      );
      return [currentIndex, gestureHandler];
    };
    /**
     * A Page component.
    ...
    const PageGestureUser = () => {
      ...
      const [page, setPage] = usePagination(gesturing);
      const [transform, setTransform] = useTransform(page, gesturing);
      const gestureHandler = React.useCallback(
        gesture => {
          setGesturing(gesture.gesturing);
          setTransform(gesture);
          setPage(gesture);
        },
        [setTransform, setPage],
      );
      const gestureConfig = React.useMemo(() => ({preventDefault: true}), []);
      const ref = useGesture(gestureHandler, gestureConfig);
      return (
        <div
          tabIndex={1}
          ref={ref}
        ...
        >
          <animated.div style={{height: '100%', ...transform}}>
            {Array.apply(null, Array(TOTAL_PAGES)).map((_, i) => (
              <Page key={i} label={i} active={i === page} />
            ))}
          </animated.div>
        </div>
      );
