...
Note that `useGesture` will _not_ return a [ref object] in thie case.
...
To support these cases, `useGesture` accepts an optional
...
const SomeRefForwardingComponent = React.forwardRef((props, forwardedRef) => {
  ...
  // to `useGesture`. Note that a `ref` will _not_ be returned.
